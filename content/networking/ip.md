...
At 50% of the lease duration (the T1 timer), the client attempts unicast renewal with its original server. At 87.5% (the T2 timer), it broadcasts a rebind request to any available server. This two-stage renewal mechanism ensures graceful address reclamation when devices leave the network[^droms1997]. When a client and server reside on different broadcast domains, a **DHCP Relay Agent** on an intermediary router forwards discovery messages as unicast, allowing a single server to serve multiple subnets without a server in each one.
